...
for dimension in grouping_dimensions:
    if dimension in data.columns:
        grouped_data = data.groupby(dimension, observed=True)['Profit'].agg(['sum', 'mean']).reset_index()
...
if 'Product_Category' in data.columns:
    profit_by_category = data.groupby('Product_Category', observed=True)['Profit'].sum().sort_values(ascending=False)
...
if 'Product' in data.columns:
    profit_by_product = data.groupby('Product', observed=True)['Profit'].sum().sort_values(ascending=False).head(top_n)
...
if 'Device_Type' in data.columns:
    profit_by_device = data.groupby('Device_Type', observed=True)['Profit'].sum().sort_values(ascending=False)
...
if 'Customer_Login_type' in data.columns:
    profit_by_login_type = data.groupby('Customer_Login_type', observed=True)['Profit'].sum().sort_values(ascending=False)
...
if 'Month' in data.columns and 'Profit' in data.columns:
    monthly_profit = data.groupby('Month', observed=True)['Profit'].sum().reset_index()
...
if 'Quarter' in data.columns and 'Profit' in data.columns:
    quarterly_profit = data.groupby('Quarter', observed=True)['Profit'].sum().reset_index()
...
for variable in numerical_variables:
    if variable in data.columns:
        correlation = data['Profit'].corr(data[variable])
...
for variable in numerical_variables:
    if variable in data.columns:
        ...
        sns.scatterplot(x=data[variable], y=data['Profit'])
